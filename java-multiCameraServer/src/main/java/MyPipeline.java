...
public class MyPipeline implements VisionPipeline {
  ...
  public void process(Mat mat) {
    ...
    //draw the bounding rectangles and the contours
    for (int i = 0; i < contours.size(); i++) {
...
// Imgproc.HoughLines(edges, lines, 1, Math.PI/180, 150); // runs the actual
// detection
...
// Imgproc.HoughLinesP(edges, linesP, 1, Math.PI/180, 50, 50, 10); // runs the
// actual detection
// // Draw the lines
// for (int x = 0; x < linesP.rows(); x++) {
// double[] l = linesP.get(x, 0);
// Imgproc.line(colorEdges, new Point(l[0], l[1]), new Point(l[2], l[3]), new
// Scalar(0, 0, 255), 3, Imgproc.LINE_AA, 0);
// }
// Imgcodecs.imwrite("/home/pi/photos/coloredgesprobablistic.jpg", colorEdges);

// List<MatOfPoint> contours = new ArrayList<MatOfPoint>();
// Mat hierarchy = new Mat();
// Imgproc.findContours(edges, contours, hierarchy, Imgproc.RETR_TREE,
// Imgproc.CHAIN_APPROX_SIMPLE);
// Mat drawnContours = Mat.zeros(edges.size(), CvType.CV_8UC3);
// Rect[] boundRect = new Rect[contours.size()];
// for (int i = 0; i < contours.size(); i++) {
// Scalar color = new Scalar(0, 0, 255);
// Imgproc.drawContours(drawnContours, contours, i, color, 2, Core.LINE_8,
// hierarchy, 0, new Point());
// Imgproc.rectangle(drawnContours, boundRect[i].tl(), boundRect[i].br(), color,
// 2);
// }
